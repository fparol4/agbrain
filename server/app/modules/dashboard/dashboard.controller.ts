...
import { GetGeneralDashboardUseCase } from '#modules/dashboard/use-cases/get_general_dashboard.use_case'
import type { HttpContext } from '@adonisjs/core/http'

export default class DashboardController {
  async index({ request, response, auth }: HttpContext) {
    const { year } = await request.validateUsing(dashboardValidator)
    const dashboard = await new GetGeneralDashboardUseCase().execute(
      auth.getUserOrFail(),
      year,
      request.id()
    )
